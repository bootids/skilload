...
* Adapters 在当前切片中实现 XDG/config 文件、受限可移植文件传输与 bundled SQLite Library repository；SQLite adapter 独占 schema v2 的 base/derived 两层维护：FTS5 索引在同一 transaction 中随 import/metadata mutation 显式更新，migration 与 doctor repair 从 base rows 完整重建。所有对已存在 database 的读取先通过经过 root identity 重验的 no-follow `data/skilload` directory descriptor，以相对 `openat(..., O_NOFOLLOW|O_NONBLOCK)` 持有并验证 regular `skilload.db` generation，再交给 SQLite；因此 directory、main-file、symlink 或 FIFO replacement 不会被读取或使 CLI 等待。`skilload-core` 默认拒绝 unsafe code；唯一局部审计例外是 first-import staging 与既有 database read-write connection 的 SQLite main-file `HAS_MOVED` 检查，必须在任何 SQL 前返回 identity drift，不能成为一般 native I/O abstraction。immutable cache、system Git、GitHub metadata HTTP、time/randomness 和 Claude/Codex adapters 仍为 planned.
* 已存在 database 的 read-only generation gate 把 main file 与同名 `-journal`、`-wal`、`-shm` 都视为同一 observed generation：只接受无 sibling 的 standalone DELETE-journal generation；任一 sibling 或非 DELETE header 都在 SQLite open 前返回 typed `database_corrupt`，因此 descriptor-bound read、human/JSON read 与 default doctor 均不创建 state 或混读 generation。
