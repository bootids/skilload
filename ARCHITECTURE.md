...
**Source and Trust** normalize GitHub coordinates, obtain repository identity, extract inert Git objects, validate a complete Skill tree, compute integrity, and authorize one exact source. They do not decide Library metadata or deployment targets. See `SKL-SRC-*` and `SKL-TRUST-*`.
...
**Agent adapters** resolve roots, inspect disable/conflict state, and describe link/copy actions. They never parse business CLI arguments or make policy decisions about confirmation.
