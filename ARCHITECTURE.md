...
10. No external executable is discovered or probed through an empty, relative, current-project, source-staging, or external-cache search location; candidate identity is checked before probe and revalidated before use. Every Git child drops inherited `GIT_*` state, fixes a separately validated exec path, and adds back only application-owned settings; repository/index inspection also binds explicit resolved resources. Git SSH transport forces the independently resolved SSH identity. Agent roots never derive from relative environment values (`SKL-SRC-016`, `SKL-WSP-022`, `SKL-WSP-025`, `SKL-MGR-006`).
