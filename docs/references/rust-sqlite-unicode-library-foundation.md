...
* `rusqlite 0.40.2` 没有安全的 connection-main-file identity API；其 raw `Connection::handle` 可与 bundled `sqlite3_file_control(..., SQLITE_FCNTL_HAS_MOVED, ...)` 查询 SQLite 实际打开 inode 是否已被 pathname 替换。locked `libsqlite3-sys 0.38.2` 的 Unix VFS 在 `databaseIsUnmoved` 中会在非空 database 写 journal 前自动执行同一检查，但 zero-size first-import staging 会跳过该路径。因此 `skilload-core` 只在 first-staging connection 的任何 SQL 前以一个带安全论证的局部 FFI helper 执行该 check；其他 crate code 继续由 `deny(unsafe_code)` 约束。
...
* first-import connection identity 的 FFI helper 必须只传入 `Connection` 所有的 live SQLite handle、静态 NUL-terminated `main` 名称和仅在同步 call 存活的 `c_int` out parameter；任何非 `SQLITE_OK` 或 moved result 都是 `database_identity_drift`。不得把 raw handle 暴露给 domain、application、CLI 或一般 adapter API。
