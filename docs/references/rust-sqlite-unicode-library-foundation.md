...
* `rustix::fs::renameat`、`renameat_with`、`linkat` 与 `openat` 必须仅接收已通过 no-follow 打开并经 device/inode 重验的父目录 handle，以及无分隔符的 staging/database/output 文件名；将任一绝对或未绑定 path 传给它会恢复被本交付禁止的路径重新解析窗口。export 对 absent target 使用 `RenameFlags::NOREPLACE`，在 final hook 后不创建 visible guard 地发布完整 document；对 existing target 则在 exchange 前重验 held staging FD 与 parent-relative publication entry。first import 的 `linkat` 仍在 source pathname 与 syscall 间存在未解决的 same-account replacement window，不能被额外 pathname stat 伪装为 held-FD publication；该风险必须保持为明确的 review/product decision，而不是由 cleanup 删除 foreign target。既有 database durability sync 必须经 held directory descriptor 的 no-follow `openat`、file sync、entry revalidation、directory sync 与最终 revalidation 完成。
