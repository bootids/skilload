...
* `PRAGMA integrity_check` returns `ok` when its database checks find no error, but it does not detect foreign-key violations. `PRAGMA foreign_key_check` returns one row per violation, so backup validation needs both forms of evidence when using the external SQLite CLI. Table-scoped `integrity_check(<table>)` only proves the nominated table and its indexes: a 2026-08-22 local probe created, populated, then dropped an AUTOINCREMENT table and observed that `sqlite_sequence` persisted as `CREATE TABLE sqlite_sequence(name,seq)`. Exact schema inventory must recognize that SQLite-owned table and run its own scoped integrity check rather than filtering the entire `sqlite_*` namespace.
...
* 同一 268,435,456-byte ceiling也约束 migration 自己将要发布的 backup：adapter在创建 staging 前检查 source 的 `page_count × page_size`，并在每个 512-page online-backup step后检查 SQLite 报告的 page count，staging length 在 SHA-256 前再次检查。超过预算时返回 `migration_backup_too_large`，不 hash、不发布 recovery pair、不升级 live schema；这避免“可迁移但其刚创建的 backup 又不被 validator承认”的矛盾。v1 migration baseline还必须拒绝每个非 fixed Library/recognized SQLite-owned object的 `sqlite_master` entry；`sqlite_sequence` 是允许但必须 scoped-integrity-checked 的 persistent internal table，SQLite autoindex 仅在它无 SQL definition且指向允许 owner时保留。extra table/index/view/trigger即使自身尚未被查询，也不能进入 backup 或 schema upgrade。
