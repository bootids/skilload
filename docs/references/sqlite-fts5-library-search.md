...
* FTS5 的 `integrity-check` special command 会验证索引内部一致性；对于 ordinary content-bearing table，它也比较索引和该 FTS table 自己保存的内容。命令形式是向 table 同名 hidden column 执行 `INSERT ... VALUES('integrity-check')`，所以 read-only SQLite connection 会拒绝它，即使成功检查不改变产品数据。默认 doctor 与 `library search` 必须先读取并检查 live snapshot 的 `page_count × page_size` 是否不超过 536,870,912 bytes；超限时不复制 generation，也不读取或 materialize FTS content projection。只有预算内 generation才验证 base/FTS content projection、作 in-memory backup并运行 special check。超限、或 special check 发现 drift 时一律不信任 search count/page，归为可修复的 `library_fts_invalid`。
