...
对当前 `data/skilload.db`，main file 与同名 `-journal`、`-wal`、`-shm` 是一个 observed database generation。任何 read 或 default doctor 遇到这些 sibling，或发现非 skilload 发布的 DELETE-journal header，都 MUST 在打开 SQLite 前返回带既有 recovery details 的 `database_corrupt`，不创建、修改或混读 generation member。这是对本 Revision 既有 corruption/recovery 语义的澄清，不改变 Revision。
