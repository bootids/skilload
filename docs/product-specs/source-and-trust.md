...
**Behavior.** Source validation MUST default to at most 2,000 materialized entries and 52,428,800 total bytes (50 MiB) per selected Skill before cache promotion. The entry count includes regular files and symlinks but not implicit directories. Total bytes are the exact Git blob sizes of regular files plus preserved symlink-target text. The independent defaults remain in force unless the request explicitly supplies an unsigned 64-bit ceiling through `--max-source-files <COUNT>` and/or `--max-source-bytes <BYTES>`; an omitted dimension keeps its default. JSON previews and results MUST expose both active values as lossless API-v1 `DecimalU64` strings in `source_limits.max_files` and `source_limits.max_bytes`. An override MUST be at least the corresponding default, applies only to that request and every selected source in its preview plan, is bound into any preview/confirmation token, and MUST NOT persist in Trust, Library, workspace, lock, global, or configuration state. These selected-tree overrides MUST NOT raise or disable the independent fixed acquisition budgets in `SKL-SRC-016`; a source that exceeds either layer fails with that layer's typed error.

**Acceptance.** A 2,001-entry or 52,428,801-byte candidate fails without override and `source_limit_exceeded` reports both measured values and both active ceilings in one `SourceLimitDetails`, including a dimension that remains within its ceiling. The same exact candidate may proceed only when every exceeded dimension has a sufficient explicit numeric override and any required confirmation is completed. An override of 9,007,199,254,740,992 is accepted and round-trips as the exact decimal string, while 18,446,744,073,709,551,616, a negative value, or a value below the corresponding default fails as usage/domain validation without source acquisition. A later refetch or update receives the Revision 1 defaults again unless that new request repeats the override.
...
**Behavior.** Active Trust MUST be required for every action that acquires external bytes; promotes, creates, replaces, repairs, or restores cache/deployment content; advances or changes a pin; or otherwise uses external content as valid. A sync or recovery plan proved wholly subtractive under `SKL-WSP-022` MAY proceed without active Trust only when it removes exact owned links plus their selected durable associations and performs no create, replace, repair, restore, fetch, settings inspection/mutation, or other content use. Removal, uninstall, and cleanup of already managed state MUST remain possible without active Trust. An additive, mixed, inaccessible, foreign, or drifted plan remains blocked atomically when any affected source lacks active Trust.

**Acceptance.** A batch containing one untrusted source makes no version or additive deployment changes. After revoking a deployed source and removing it from workspace desired state, an empty exact-owned sync can remove the stale link without network, content validation, or an Agent executable, after which workspace delete can succeed. The same sync remains blocked without change if it would keep/create/restore content, cannot inspect one target, or finds drift/foreign state.

## SKL-TRUST-007 - Revocation behavior (Revision 1)

**Behavior.** Revoking Trust MUST preserve current verified managed links and all unrelated state, but MUST block future update, pin, every sync or recovery action that acquires, creates, replaces, repairs, restores, or otherwise uses external content, and every cache-miss restoration for that source. Revocation alone MUST NOT execute or delete source content. A later explicitly requested, fully proved subtractive sync/removal MAY delete the exact managed link and association under `SKL-TRUST-006`; it does not make the revoked content valid again.

**Acceptance.** Immediately after revoke, an existing linked Skill remains available. Its next restricted mutation returns a structured trust-required error, while explicit uninstall/remove and a proved empty-workspace cleanup sync succeed without reacquiring or validating the revoked content.
