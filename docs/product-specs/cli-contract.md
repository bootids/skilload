...
**Behavior.** Human-facing 0.1 output MUST be English. Primary results and help go to stdout; warnings, progress, and diagnostics go to stderr. Noninteractive terminals MUST receive usable output without relying on color or cursor control.

**Acceptance.** Snapshot tests cover both TTY and non-TTY modes, with equivalent meaning and no required ANSI sequences. JSON stdout remains unaffected by stderr diagnostics.
