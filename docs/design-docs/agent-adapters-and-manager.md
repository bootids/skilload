...
Native Mach-O/ELF files end resolution. For a script, read at most 4,097 first-line bytes and require a newline or file EOF within the first 4,096. Parse only the exact byte tokens from `SKL-WSP-022`: a direct `/[A-Za-z0-9._+/-]+` interpreter with an optional single-space `[-A-Za-z0-9._+/=:,]+` literal argument, or `/usr/bin/env` plus one `[A-Za-z0-9][A-Za-z0-9._+-]*` basename. Reject NUL, carriage return, tabs, relative direct interpreters, every other spacing/token, and an env option, assignment, path, or extra operand including `env -S`. Resolve the named interpreter through the same safe absolute-candidate algorithm; the env form is a lookup marker and launches the resolved basename directly rather than executing `env`. If that result is another script, recurse while retaining identities. Reject a cycle or a fifth script identity. Construct the final argv vector explicitly without a shell by folding each script layer into `[interpreter, optional_argument, script_path, prior_arguments...]`; the terminal native executable is `argv[0]`. Provide a PATH made only from resolver-accepted absolute directories for child behavior as well as resolution. Return `ResolvedLaunch { argv, executable_chain, path_directories, source }`; each chain member records its canonical path and file identity. Fail with `unsupported_interpreter` before running the original candidate when the chain is ambiguous or unsupported, `unsafe_executable_path` when a member is contained by an untrusted root, or `executable_not_found` when no safe required member exists. JSON path details use `PathValue`.

Revalidate every recorded executable/interpreter identity and the resolver-built PATH immediately before every spawn and again at mutation commit when the launch contributed to the plan. Run a bounded documented noninteractive version/help probe, when needed, from a private empty working directory with repository-sensitive environment inputs removed and stdin closed; never start an interactive Agent. Record the complete canonical launch in the plan but do not require a numeric Agent version. The same-account concurrent-replacement exclusion in the threat model permits identity revalidation rather than claiming an unavailable cross-platform execute-by-handle primitive. The accepted grammar and operating-system rationale are preserved in [`../references/executable-script-resolution.md`](../references/executable-script-resolution.md).

Repository-controlled Agent settings and conflict roots are also untrusted. Before settings deserialization, one selected Agent may contribute at most 64 settings documents, 1,048,576 bytes per document, 8,388,608 aggregate bytes, 100,000 scalar/container nodes, 16 container levels, and 65,536 UTF-8 bytes per scalar. A streaming `agent-project-input-v1` pass rejects duplicate keys, aliases/anchors, explicit tags, include/import expansion, multiple documents, and non-string keys wherever the input format can express them. Conflict inventory uses `lstat`, never follows a directory symlink, and stops before entry 100,001, directory level 33, candidate Skill root 10,001, a 256th native-byte segment, or a 4,097th native-byte relative path. It applies the `SKL-SRC-007` frontmatter limits to each candidate before building that candidate's model. Any exceeded dimension returns `agent_input_limit_exceeded { input_class, measured, allowed }` with no partial settings/conflict observation or action plan.
...
4. Run `agent-project-input-v1` over every project-controlled settings document, then load only the accepted bounded subset needed to detect disabled names without modifying it.
5. Inventory exact and semantic conflicts under the bounded no-directory-symlink traversal, including reserved/bundled names where the Agent exposes them.
...
After root and exact-target observation, the application may select removal-only mode only when the whole action plan is subtractive: an empty workspace sync, normal global uninstall, manager uninstall, cache cleanup, or equivalent journal recovery removes exact owned entries and selected durable associations without creating, replacing, repairing, restoring, fetching, treating source content as valid, or inspecting Agent settings/conflicts. In this mode the Agent executable may be absent, revoked Trust does not block the explicitly requested exact cleanup, and steps 4-5 above are omitted; root accessibility, parent safety, target digest/link/marker evidence, and final drift revalidation remain mandatory. Any additive, content-using, or mixed action switches the entire selected plan to normal functional preflight and active-Trust gating. An absent Agent or revoked approval therefore cannot strand exact-owned cleanup, while an inaccessible, modified, or foreign target still cannot be deleted.
...
Use fake Agent executables and isolated HOME/config roots. Resolver matrices cover both exact Agent configuration keys; absolute safe and missing overrides; empty, relative, and absolute PATH entries; project/worktree/cache containment; outside symlinks back into those roots; final non-files/executable permission; identity drift; private probe CWD/environment; and the same cases for Git, optional `gh`, SSH, and Agent-visible `skilload`. Script fixtures cover the 4,096-byte first-line boundary, zero/one direct argument, `/usr/bin/env <basename>`, `env -S`, relative interpreters, assignments/options/extra operands, shell metacharacters, cycles, four/five-script chains, unsafe nested interpreters, and identity drift. Workspace interpreter markers must never execute. Adapter contract tests cover unset/empty/relative/absolute `HOME`, `CLAUDE_CONFIG_DIR`, and `CODEX_HOME` from multiple current directories; default/override roots; deprecated Codex conflict discovery; disabled settings; parent symlinks; exact foreign targets; semantic conflicts; same-root observation refresh versus changed-root identity; fixed-workspace `CODEX_HOME` changes; global environment profile changes; and next-launch fixture discovery. Agent-input fixtures exercise every exact document/byte/node/depth/scalar and traversal entry/depth/candidate/segment/path boundary, every rejected expansion feature, directory symlinks, and bounded candidate frontmatter; no failure may return a partial observation or create an action. Removal matrices uninstall each Agent executable first, then require empty workspace sync, global uninstall, and exact manager uninstall to succeed without a probe; additive/mixed, inaccessible, modified, and foreign cases remain atomic failures. Manager tests parse both embedded assets, compare referenced commands and API payloads to the authoritative API-v1 catalog, validate install-time and runtime PATH rules, exercise multi-Agent failpoints, and prove cache clear cannot remove manager copies. Live model conversations remain optional nonblocking smoke tests.
