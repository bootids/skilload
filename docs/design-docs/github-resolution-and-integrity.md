...
4. Set `GIT_TERMINAL_PROMPT=0`, a dedicated empty hooks directory, restricted protocols, no optional locks outside the staging repo, and fixed config that prevents user/repository filters from materializing content. For SSH, remove inherited `GIT_SSH`, `GIT_SSH_COMMAND`, and `GIT_SSH_VARIANT`, then set a new application-owned `GIT_SSH_COMMAND` containing an injective POSIX-shell single-quoted canonical SSH path plus only fixed `-oBatchMode=yes`, and set `GIT_SSH_VARIANT=ssh`. Git documents that `GIT_SSH_COMMAND` takes precedence over `GIT_SSH` and `core.sshCommand`, so this owned value prevents caller/global configuration from selecting a different local program. Revalidate the SSH file identity immediately before spawning Git and pass a PATH containing only already accepted absolute search directories.
