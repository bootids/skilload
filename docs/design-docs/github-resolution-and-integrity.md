...
Use system `git` because it is an explicit runtime dependency, but resolve it before any probe through the shared external-executable adapter from the Agent design: skip empty/relative PATH entries, reject candidate/symlink targets in the current project/worktree or skilload source/cache roots, require a regular executable, and retain its canonical identity for revalidation. Build every Git child environment from an explicit allowlist of required non-Git operating-system, sanitized PATH, proxy/TLS, locale, HOME, and authentication-agent values; never copy a caller variable whose name begins `GIT_`. With that clean environment and a private empty current directory, invoke the selected binary as `git --exec-path`, require one nonempty absolute result, canonicalize and reject a result inside any identified project/worktree or skilload source/cache root, and record the directory identity. Every later Git invocation revalidates both identities and passes `--exec-path=<canonical-directory>` before the subcommand. This prevents inherited `GIT_EXEC_PATH` and PATH from selecting a remote helper or other dashed Git subprogram. Resolve optional `gh` by the shared rule. Before an SSH attempt, independently resolve fixed basename `ssh` by that rule; do not attempt SSH if only unsafe or missing candidates exist. Avoid a normal working-tree checkout. For each acquisition:

1. Create a new private temporary directory and bare repository.
2. Revalidate the selected Git file and exec-directory identities and invoke its canonical absolute path with the fixed `--exec-path` directly as an argv array, never through a shell.
3. Construct the GitHub remote URL from validated owner/repository, not arbitrary transport input.
4. Starting from the clean allowlist, add only application-owned Git settings: `GIT_TERMINAL_PROMPT=0`, a dedicated empty hooks directory, restricted protocols, no optional locks outside the staging repo, and fixed config that prevents user/repository filters from materializing content. Caller `GIT_CONFIG_*`, `GIT_DIR`, `GIT_WORK_TREE`, `GIT_INDEX_FILE`, `GIT_OBJECT_DIRECTORY`, `GIT_ALTERNATE_OBJECT_DIRECTORIES`, `GIT_COMMON_DIR`, `GIT_NAMESPACE`, and every other inherited `GIT_*` value are absent. For SSH, add a new application-owned `GIT_SSH_COMMAND` containing an injective POSIX-shell single-quoted canonical SSH path plus only fixed `-oBatchMode=yes`, and set `GIT_SSH_VARIANT=ssh`. Git documents that `GIT_SSH_COMMAND` takes precedence over `GIT_SSH` and `core.sshCommand`, so this owned value prevents caller/global configuration from selecting a different local program. Revalidate the SSH file identity immediately before spawning Git and pass a PATH containing only already accepted absolute search directories.
...
For every candidate, locate the closing frontmatter delimiter without reading beyond 65,536 bytes, require UTF-8, and feed a non-expanding event API rather than a deserializer that resolves aliases. Abort before constructing the model on node 513, container level nine, scalar byte 4,097, any anchor/alias/explicit tag, duplicate key, or non-string key. Require a `name` of 1 through 64 ASCII bytes matching `^[a-z0-9]+(?:-[a-z0-9]+)*$`, exact equality with the selected source directory's last Git segment (or normalized repository name at root), and a nonempty description of at most 1,024 Unicode scalar values. Store `SkillName` as already validated lowercase ASCII and compare it byte-for-byte; never trim, case-fold, or Unicode-normalize. The verified name becomes the deployment directory name. Do not execute dynamic content, referenced scripts, or frontmatter extensions. Candidate preview includes validation warnings for Agent-specific optional fields but skilload does not translate them. The cross-Agent basis is recorded in [`../references/claude-and-codex-skill-discovery.md`](../references/claude-and-codex-skill-discovery.md).
...
Default tests use local bare Git repositories and an HTTP fixture server that models GitHub responses, redirects, authentication failures, mutable refs, deleted commits, and rate/error conditions. Fixtures cover the exact discovery, candidate, entry, byte, frontmatter-event, name, and description boundaries; explicit-path discovery bypass; one-shot per-dimension overrides; hostile paths/modes/YAML; filters/hooks that must never execute; submodules; LFS pointers; symlink graphs; candidate ambiguity; canonical delimiter collisions and percent round trips; repository path reuse; confirmation replay/drift; hostile terminal fields; and byte-exact golden integrity digests with eight-byte lengths. Process fixtures place marker helpers under inherited `GIT_EXEC_PATH`, inject dynamic config/repository/object selectors, and prove the clean runner plus fixed validated exec path reaches none of them. SSH fixtures put marker executables in relative/project PATH entries and caller `GIT_SSH`/`GIT_SSH_COMMAND`, then prove only the revalidated safe fixture receives Git's host/command arguments through the application-owned command and variant. Migration fixtures prove `source migrate` reports but does not mutate workspace ownership/config/lock evidence and that the later journaled workspace operation sees the original exact source. Real GitHub smoke tests are explicit or scheduled and never required for the default suite.
