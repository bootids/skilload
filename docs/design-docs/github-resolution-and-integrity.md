...
All outputs carry a `ResolutionEvidence` value with canonical source, repository ID, commit, verified name/description, integrity, counts, warnings, the ordered metadata/content attempt records that later compose per-source `NetworkUse`, and staged cache key.
...
If repository-only input yields more than one valid candidate, return all candidates sorted by path. A human caller selects explicitly; JSON returns typed source-kind locators and no confirmation token until one source identity is exact. Repository/ref failures before path discovery use repository/ref-kind locators with `path: null`, not the root-Skill sentinel `path: ""`.

## Tree Validation

Walk only entries under the selected root. The Revision 1 defaults allow 2,000 materialized regular-file/symlink entries and 52,428,800 bytes, where byte accounting sums regular blob sizes and symlink-target blob sizes. In one bounded Git-metadata traversal, use checked `u64` arithmetic to measure both dimensions before allocating blob content; exceeding one dimension suppresses content allocation but does not short-circuit the other measurement. `--max-source-files` and `--max-source-bytes` map to an optional application `SourceLimits { max_files: u64, max_bytes: u64 }`; validate each supplied unsigned-64-bit value against its default, keep the default for an omitted dimension, and scope the resulting ceilings to the request's selected source set. JSON previews/results serialize the active values as lossless decimal strings. A validation failure returns one `SourceLimitDetails` containing both measured/allowed file and byte dimensions, not whichever check happened first; checked-sum overflow fails before content allocation rather than wrapping. Bind the complete override and every affected source into the preview and confirmation token but persist neither it nor an unlimited marker. It never raises the fixed pack-receive budgets above. A later update, refetch, or cache-miss restoration therefore requires a new explicit tree-limit override when the exact selected content exceeds a default and can still fail independently at acquisition.
