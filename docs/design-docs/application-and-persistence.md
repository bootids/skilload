...
Schema v2 的 read 兼容性与 doctor 语义已由 `PLAN-0005` 实现：所有对已存在 database 的打开先经 pre-open generation gate（main-file 100-byte header 的 journal-mode bytes 非 (1,1) 或存在 `-wal`/`-shm` sibling 即按 `database_corrupt` 拒绝，不创建任何 sidecar）；默认 `doctor` 以 read-only source connection 做 online backup 到 `:memory:`，在副本上运行需要 writable connection 的 FTS5 `integrity-check`，live filesystem 保持不变；`doctor --fix` 只对已证明 base rows 完整的 v1 migration 或 FTS-only drift 执行 action。v1 migration 在 `data/backups/` 发布 standalone database 与 completed sibling manifest（SHA-256、字节数、source identity、epoch-nanosecond 时间戳）后才在 live connection 的一个 transaction 中创建/填充 FTS 并更新 schema version；migration 前后 `state_revision` 不变，并保守 prune 超过三代之外的完整 validated backup pairs。
