...
当前 sidecar 的 no-follow FD identity 只证明 scan 时 entry 与 held FD 相同，不能证明同账号并发方未在 scan 前创建该 inode；因此 foreign-sidecar deletion 与 pre-commit absent-root cleanup 的取舍仍是 P2 review 中明确记录的待决产品/架构问题，不能把重复 pathname observation 当作 provenance。
...
发布 regression 必须证明 first import 在 live name 出现时已经是 committed SQLite generation，并在 link 后、final identity check 后的 database replacement 中保留 foreign target；source-path replacement 前的 held-FD provenance 仍为待决问题。sidecar tests 同时证明每个 SQLite persistence failure path 的已记录 sidecar 被清理，而 pre-commit foreign sidecar 保留；它们不得声称仅凭 scan identity 已证明 sidecar creation provenance。

## Decisions Deferred Beyond the Configuration Foundation

`PLAN-0002` 固定 Rust toolchain、`clap`、`serde`、TOML、error、filesystem-staging、JSON 与 test dependencies。`PLAN-0003` 将 `rusqlite 0.40.2`（`default-features = false`、`bundled`）、`unicode-normalization =0.1.23`、`libc =0.2.189` 与 `rustix =1.1.4`（`fs`）锁入 workspace，并固定 P2 SQL names 与 local Unicode 15.1.0 数据；`rustix` 提供 first-import no-clobber `linkat`、export existing-target `RenameFlags::EXCHANGE` 与 absent-target `renameat_with(..., RenameFlags::NOREPLACE)`，不引入新的 domain 或 general I/O abstraction。HTTP、forward migration/backup、FTS schema 与其余 durable-domain dependency decisions 继续由后续交付决定。
