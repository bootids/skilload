...
3. 在 forward migration 前，先以 current source 的 checked `page_count × page_size` 证明 standalone backup不超过 268,435,456 bytes，再通过 SQLite backup API 在 `data/backups/` 创建 standalone durable backup，而不是逐字节复制 live WAL database。`backups` child必须从已经 held 且 identity-validated 的 `data/skilload` descriptor相对创建、打开并同步 parent directory entry；final pair的 `linkat` publication也只使用该 held child descriptor。backup每个 512-page `More`、`Busy` 或 `Locked` step以既有 `LOCK_RETRY` 暂停，累计超过 `LOCK_WAIT` 返回 typed database `busy`；每步后仍重验 reported page count，staging descriptor length在 SHA-256 前再次受同一上限约束。任一步超过上限或重试预算返回 error，不 hash、不发布 pair、不写 live schema。database/manifest完成 publication、directory sync和测试 hook后，adapter必须再次证明 held directory及两项 final entry identity未变，才可开始 live schema transaction。名称包含 source schema、target schema 与 UTC creation time；sibling manifest包含 schema versions、byte size、SHA-256 digest、source database file identity 和 completed marker。
   migration 的每次 v1 baseline validation 还要求 exact non-FTS `sqlite_master` inventory：只允许四个 fixed base tables、v2 的 owned FTS virtual/shadow names，以及明确识别的 SQLite-owned layout companions。宽泛排除 `sqlite_*` namespace 是不允许的：删除 AUTOINCREMENT table 后仍保留的 `sqlite_sequence` 必须作为允许的 persistent internal table 并纳入 table-scoped `integrity_check`；SQLite autoindex 只在其无 SQL definition 且 owner 是允许 table 时可出现，且由该 owner 的 scoped check 覆盖。任何 extra table、index、view 或 trigger 都是 `database_corrupt`。第一次 proof发生在发布 standalone backup 前，第二次发生在最终 version update 的同一 SQLite transaction 内，因此异常 schema object不能把已验证 Library rows 改写后误报 migration success。已知 FTS name的 malformed/missing tolerance仍只属于 schema v2 derived repair，不放宽 v1 migration inventory。
