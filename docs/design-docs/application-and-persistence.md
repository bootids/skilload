...
3. 在 forward migration 前，通过 SQLite backup API 在 `data/backups/` 创建 standalone durable backup，而不是逐字节复制 live WAL database。名称包含 source schema、target schema 与 UTC creation time；写入 sibling manifest，包含 schema versions、byte size、SHA-256 digest、source database file identity 和 completed marker。
   migration 的每次 v1 baseline validation 还要求 trigger-free base schema：第一次发生在发布 standalone backup 前，第二次发生在最终 version update 的同一 SQLite transaction 内，因此异常 `schema_info` trigger 不会把已验证 Library rows 改写后误报 migration success。
...
即使 FTS content-row projection 与 base rows 相等，search 仍在 count/page 之前执行 special integrity check；因此 zeroed or logically damaged inverted-index block 不能返回 successful empty result。`doctor --fix` 持锁后的健康重诊直接在 writable connection 运行该检查，避免对 oversized generation 再创建 in-memory copy。
...
默认 doctor 与 search 的 read-only special-check copy 使用 536,870,912-byte checked page budget；超限 generation 保持 base rows 可读，但 FTS 作为 `library_fts_invalid` 交给显式 `doctor --fix`。这避免外部增大的 FTS shadow/freelist 在 default diagnostic 中按数据库全量分配进程内存。
