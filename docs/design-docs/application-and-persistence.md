...
* `library_tags`: many-to-one tags with the Unicode-15.1 NFC display spelling and unique full-case-folded comparison key required by `SKL-LIB-008`；v1 base schema 的完整 foreign-key inventory只允许 `library_tags` 的唯一 `canonical_source → library_entries.canonical_source ON UPDATE NO ACTION ON DELETE CASCADE MATCH NONE` relation；`schema_info`、`state_revision` 与 `library_entries` 不得有 foreign key。读路径验证完整 inventory而不只依赖 `foreign_key_check`，因此 migration 的 version update不会触发未验证 cascade。
...
Schema v2 的 `library_fts` 保存 `canonical_source UNINDEXED`，并分别索引 name、description、alias、tag display spellings、tag comparison keys、category、note 和 repository。它不使用 external-content triggers：Library adapter 在新增或 metadata mutation 的同一 SQLite transaction 中显式替换一个 entry 的派生 FTS row，migration 与 `doctor --fix` 则从 base rows 完整重建。Base metadata始终保留原始 UTF-8；共享 projection helper 对非 NFC 的 free-text column 保留原值并以 ASCII newline 追加 NFC representation，随后再追加 distinct 的完整默认大小写折叠后 NFC representation；newline 是 tokenizer separator，因此 normalized 与 expanding full-fold query term都可命中等价值而不改变 list/get/export 输出。这样 tag 聚合只有一套 Rust/SQL 路径，FTS corruption 也可在不改变 `state_revision` 的情况下丢弃并重建。Tokenizer 固定为 bundled FTS5 的 `unicode61 remove_diacritics 0`；产品查询先按本地 Unicode 15.1.0 `White_Space` 分词，对每个词项生成 NFC 原文与完整默认大小写折叠后的 NFC alternative，再逐项做 FTS5 string quoting，在同词项 alternatives内用 parenthesized OR、在不同词项之间显式 `AND`；用户输入绝不拼接为 FTS grammar。
...
Schema v2 `library search` 也必须在任何 `MATCH` count 或 page query 前，以 read-only snapshot 的 bounded in-memory backup 运行同一 FTS5 special integrity check；即使 FTS content projection 可读、inverted-index block 让 MATCH 静默返回零，也必须返回 `library_fts_invalid`。在读取或 materialize 任一 FTS content row 前，adapter先检查 page count 与 page size 的 checked product；超过 536,870,912 bytes 时不复制 generation、也不做 derived content comparison：default doctor报告不可离线验证的 error finding `library_fts_diagnostic_snapshot_too_large`，`doctor --fix` 不写入也不把它误报为已修复；search继续以既有 `library_fts_invalid` 拒绝未验证 index。这保留 default doctor/search 对 live XDG state 的 filesystem-inert 约束并避免无界内存分配。
...
1. 验证文件类型、可用时的受限 ownership/permissions、SQLite header、integrity status 和 schema version。对已存在 database，先在 root-resolution anchors 前后验证 `data/skilload`，以 no-follow directory descriptor 持有该目录；read-only gate 只通过该 descriptor 的 `openat(..., O_RDONLY|O_NOFOLLOW|O_NONBLOCK)` 打开单组件 `skilload.db`，以 `fstat` 证明 regular file、以 `statat` 证明仍是 held directory entry，再从同一 file descriptor 检查 header。SQLite read-only connection 从该 descriptor 的 `/dev/fd/<fd>` 打开；任意 SQL 前后均重验 held data directory、entry 与原 pathname identity，因此 root directory、main-file、symlink、FIFO 或 ABA replacement 都不能使 gate 与实际读取的 generation 分离。
   ...
   同一 header reader把少于 100 bytes、SQLite magic不匹配或 journal-mode bytes不是 `(1, 1)` 作为 content corruption；`read_exact` 的其他 I/O error在重新验证 held directory/main-file generation后以 typed `XDG_DATA_HOME` error返回。这样 transient storage/descriptor failure不会错误地要求 operator 执行 corruption recovery。
...
3. 在 forward migration 前，adapter必须在一个 SQLite SHARED snapshot内验证 v1 baseline、读取 `state_revision` 与完整 Library entries，并将该 source transaction保持到 standalone backup已复制、验证、发布且 durable 为止；因此 recovery image与已验证 rows不能被未更新 semantic revision的外部 writer分离。先以该 source 的 checked `page_count × page_size` 证明 backup不超过 268,435,456 bytes，再通过 SQLite backup API 在 `data/backups/` 创建 standalone durable backup，而不是逐字节复制 live WAL database。`backups` child必须从已经 held 且 identity-validated 的 `data/skilload` descriptor相对创建、打开并同步 parent directory entry；final pair的 `linkat` publication也只使用该 held child descriptor。backup每个 512-page `More` 表示成功复制并立即继续；只有 `Busy` 或 `Locked` 以既有 `LOCK_RETRY` 暂停，累计超过 `LOCK_WAIT` 返回 typed database `busy`；每步后仍重验 reported page count，staging descriptor length在 SHA-256 前再次受同一上限约束。任一步超过上限或 contention retry预算返回 error，不 hash、不发布 pair、不写 live schema。database/manifest完成 publication、directory sync和测试 hook后，adapter必须再次证明 held directory及两项 final entry identity未变，才可结束 source snapshot并开始 live schema transaction；该最终 transaction还必须重新验证 schema/revision并比较完整 entries，任一 drift都拒绝 migration。名称包含 source schema、target schema 与 UTC creation time；sibling manifest包含 schema versions、byte size、SHA-256 digest、source database file identity 和 completed marker。

   standalone staging database也必须先在其 own read-only SQLite SHARED snapshot内完成 schema/base validation，再在该 snapshot保持到 DB/manifest pair link、directory sync和最终 entry revalidation完成期间计算 SHA-256并发布。另一个 connection 可以取得 RESERVED lock，但不得在这段边界提交不同 bytes；digest因此描述同一个已验证、已发布 recovery image。final pair发布后，staging names只在仍匹配 held inode时以 descriptor-relative `unlinkat` 删除；真实删除错误停止 migration并返回 typed error，不能以 success隐藏可能很大的 temporary duplicate。
   migration 的每次 v1 baseline validation 还要求 exact non-FTS `sqlite_master` inventory：只允许四个 fixed base tables、v2 的 owned FTS virtual/shadow names，以及明确识别的 SQLite-owned layout companions。宽泛排除 `sqlite_*` namespace 是不允许的：删除 AUTOINCREMENT table 后仍保留的 `sqlite_sequence` 必须作为允许的 persistent internal table 并纳入 table-scoped `integrity_check`；base table的 autoindex必须精确为 `sqlite_autoindex_library_entries_1`、`sqlite_autoindex_library_entries_2` 与 `sqlite_autoindex_library_tags_1`，并与其 fixed owner匹配，缺失、重命名或额外 base autoindex均是 `database_corrupt`。v2 derived autoindex仍只可作为已拥有 FTS layout companion出现。任何 extra table、index、view 或 trigger都是 `database_corrupt`。第一次 proof发生在发布 standalone backup 前，第二次发生在最终 version update 的同一 SQLite transaction 内，因此异常 schema object不能把已验证 Library rows 改写后误报 migration success。已知 FTS name的 malformed/missing tolerance仍只属于 schema v2 derived repair，不放宽 v1 migration inventory。
   除 object inventory外，v1 baseline还必须在 backup publication与最终 version update前验证完整 fixed foreign-key inventory：只有 `library_tags` 的固定 relation可存在，三个其余 base table的 relation数必须为零。`foreign_key_check` 只报告已发生的 violation，不能替代此声明级 proof；否则 external `ON UPDATE CASCADE` 可在 `schema_info.version` update时改写 base metadata。

   object/owner inventory之后，base proof还通过 `PRAGMA table_info` 验证四张 fixed table的完整 column order、declared type、nullability、default 与 primary-key position，并通过 `PRAGMA index_xinfo` 验证三个 fixed base key的 sequence、ascending BINARY collation与 column order。这样重建的 `library_entries` 即使保留 `sqlite_autoindex_library_entries_1` 名称，只要将 `canonical_source` 改为 `COLLATE NOCASE` 就会在 backup publication和任何 write前作为 base corruption拒绝。
...
`database_corrupt` 的 backup inventory 不把 filename、timestamp 或 digest 单独视为证明。它从 held backup-directory descriptor 以 `openat(..., O_NOFOLLOW)` 打开 pair，最多读取 4 KiB private manifest，要求 format version、source schema 1、current target schema、`complete`、size、SHA-256、directory-entry identity，并要求 manifest 的 source device/inode恰好匹配当前 diagnosed main-file generation；没有可证明 main identity的 orphan-sidecar path fail closed为 empty inventory。随后以 held standalone database descriptor在 SQLite SHARED snapshot中验证 header、schema v1 和完整 base rows，并在 snapshot前后盘点该 `.db` 的 `-journal`、`-wal`、`-shm` companion。任何 candidate 的 no-follow open、metadata、manifest read、SQLite snapshot、hash或entry revalidation operational failure必须作为 typed `XDG_DATA_HOME` I/O error传播；只有 genuinely absent、symlink/nonregular、format不兼容、source identity不匹配、corrupt或带 companion的 content candidate才是 `false` 并从 inventory排除。`Dir::read_from` 初始化或逐项读取失败同样传播，只有 genuinely absent `backups` directory才是 empty inventory。
...
backup corruption diagnostic在排序或读取任何 pair前最多保留 64 个 `*.manifest.json` stems；第 65 个返回 `library_database/backup_manifest_candidate_limit_exceeded` typed `invalid_state`，绝不返回截断 inventory或继续无界枚举。该候选数预算与每份 manifest的 4 KiB 和 standalone database的 268,435,456-byte boundaries共同限制受损 backup directory的内存、I/O与诊断时间。
...
`library export` 的 recoverable projection 只依赖完整的 `library_entries`/`library_tags` schema、逐表 integrity、foreign-key 与 domain-document validation；它不依赖已损坏的 `schema_info` 或 `state_revision`。在将 `library.export` 放入 `database_corrupt` diagnostics 前，adapter还必须以与真实 export 相同的 deterministic no-capture transfer encoder证明完整 document不超过 10,000 entries 与 67,108,864 bytes；只有该 projection实际可导出时才列出 identifier。human error renderer 与 API-v2 都显示已验证 backup paths 和该 recovery export identifier。
Recovery export probe 中，entries/tags content、domain 或 deterministic transfer-size validation失败只说明该 identifier不可广告；但 no-follow open、SQLite I/O、lock、memory、descriptor exhaustion、generation revalidation及其他 operational failure必须保持 typed error 向上传播。这样 `database_corrupt` details只在 inventory完整证明后返回，绝不以成功 envelope隐藏未完成的 asset census。
...
默认 doctor 与 search 的 read-only special-check copy使用 536,870,912-byte checked page budget，并在读取/物化 FTS content projection前执行该检查；超限 generation保持 base rows可读，search以 `library_fts_invalid` 拒绝未验证 FTS，default doctor以不可 fix 的 `library_fts_diagnostic_snapshot_too_large` error finding说明资源边界，`doctor --fix` 返回 unchanged且不改 live database。这避免外部增大的 FTS shadow/freelist在 default diagnostic中按数据库全量分配进程内存或无限重复 no-op repair。
此 snapshot budget只是 default-doctor/search 的 read-only diagnostic资源边界，不是 durable mutation gate。`DoctorData.database_writable` 因而仍表示当前 binary是否按 observed-generation classification允许 mutation：超限 finding保持不可 fix、`doctor --fix` unchanged，但字段为 true；实际 import/metadata mutation仍在各自 write transaction上运行完整 base/FTS integrity gate，并可据其真实结果拒绝损坏 database。
...
对既有 target 的成功 exchange，adapter只在 `OutputPublicationGuard` 仍证明随机 publication entry是 captured displaced output时尝试 `unlinkat`。该 cleanup 的失败必须返回 typed export error，不能报告 success；若 cleanup 前 identity已不匹配，则保留该 entry以避免删除已观察到的 foreign replacement。POSIX/Darwin/Linux没有把 prior identity check与 unlink原子合并的可移植原语，因此该最佳努力 cleanup不改变未知 replacement一律不得由 error path删除的规则。
