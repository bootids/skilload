...
若仅 `library_fts` 的 virtual-table SQL text malformed，connection-local `PRAGMA writable_schema=ON` 允许 base-only validation、list/get/export 与 default doctor 读取经验证的 base rows；search 与 write 仍返回 `library_fts_invalid`。repair 在同一 derived-only transaction 删除 FTS/shadow schema rows 后用 `PRAGMA writable_schema=RESET` reload schema，再重建固定 virtual table，绝不改 base rows 或 `state_revision`。
...
同一 protected-target inventory 还包含所有已发布 `data/backups/skilload-db-v1-to-v2-*.db` 及 matching `.manifest.json` entries；同一 inode 的 hard-link alias 也拒绝，确保 forward migration 的 standalone recovery evidence 不会被 portable document 覆盖。
