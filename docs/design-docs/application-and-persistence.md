...
对于原本不存在的 `data/skilload.db`，持久 import 在完成 input/schema/domain/conflict planning 后，在同一 data directory 建立 restrictive staging database；只有 schema 和 SQLite `COMMIT` 成功、SQLite connection 已关闭且根重新验证后才尝试发布 live database。发布先以 held data-directory descriptor 用 `O_EXCL|O_NOFOLLOW` 创建只可由本调用拥有的空 `skilload.db` guard，再用 `RenameFlags::EXCHANGE` 交换 held publication link 与 guard；交换后必须同时证明 live entry 是 held staging inode、publication entry 仍是 guard identity。任一 identity drift 都立即再交换回发布前状态并返回错误，绝不报告 success。`COMMIT` 失败后，staging 仅记录以 no-follow descriptor 绑定且仍为同一 inode 的 `-journal`、`-wal`、`-shm` sidecar；Drop 只删除这些记录的 sidecar、held staging、publication link、database lock 与空 data/state directory。pre-commit hook 或未知来源留下的同名 sidecar 不被推断为 owned，保留并阻止删除所在目录。若 `COMMIT` 已完成但文件或父目录 sync 失败，命令返回 error 且不承诺 absence。
...
所有 publish hook 或最终验证完成后，first import 必须再次比较 held staging FD、data-directory descriptor、publication link 与 database guard，才允许 `EXCHANGE`。交换后立即复验 live `skilload.db` 是 held staging、publication entry 是 guard；任一不匹配先执行反向 `EXCHANGE` 恢复原 target，再返回 typed drift error。发布后完成 parent/data-directory 与新建目录 sync 后，first import 必须最后一次比较 live `skilload.db` entry 与 held staging FD，任何 drift 都返回 typed error 而不报告 `changed`。未发布 staging 的 Drop 只按 held identity 清理原 staging、publication entry 和已记录 sidecar，再让创建目录的 RAII guard 删除仍为空且当前 identity 匹配的根；未证明属于本调用的同名 sidecar 保留。

为避免 source staging pathname 在 final check 后被替换，P2 先在 held parent 中为随机 publication name 创建并安全移除 placeholder，再以 descriptor-relative `linkat` 从已验证 staging entry 建立 held inode 的第二个 link。publication link 与 destination guard 均通过后，`EXCHANGE` 只在 post-exchange live entry 仍为 held inode 时完成；否则反向交换恢复 guard/old target。正常成功会按 held identity 删除原 staging link 与已置换的 guard。
...
P2 默认测试使用 temporary XDG/HOME roots、bundled SQLite 与 generated input；它们覆盖 Unicode 15.1.0 tag normalization、canonical source/positive portable evidence、六种 non-model import ceiling 与 API-v2 `library_input_limit_exceeded`、combined 10,000-entry/byte transfer closure、duplicate keys、nonregular/identity-drift input、first-import staging cleanup 与 foreign-sidecar preservation、pre-open 与 read-only SQLite ABA inode replacement、publication-link 与已发布 database replacement、SQLite transaction rollback、post-commit error、既有数据库 no-follow/identity race、缺失 schema 列、foreign-key parent-key mismatch 与超出 API-v2 `UInt` 的损坏诊断、state revision 溢出拒绝、deterministic export、database/WAL/SHM/lock collision、held parent descriptor 的 rename race、publication-link/output replacement、parent sync failure 与 `database_corrupt`。fake repository、FTS/query、pagination、migration/recovery、Trust 与其他 full-model fixture 仍属于后续交付。

发布 regression 还必须在最终 identity check 之后替换 export/database publication entry，证明旧 output 或 database absence 被 exchange 恢复；sidecar tests 同时证明已记录的 SQLite commit-failure sidecar 被清理，而 pre-commit foreign sidecar 保留。

## Decisions Deferred Beyond the Configuration Foundation

`PLAN-0002` 固定 Rust toolchain、`clap`、`serde`、TOML、error、filesystem-staging、JSON 与 test dependencies。`PLAN-0003` 将 `rusqlite 0.40.2`（`default-features = false`、`bundled`）、`unicode-normalization =0.1.23`、`libc =0.2.189` 与 `rustix =1.1.4`（`fs`）锁入 workspace，并固定 P2 SQL names 与 local Unicode 15.1.0 数据；`rustix` 提供安全的 descriptor-relative `linkat`、`O_EXCL|O_NOFOLLOW` guard 与 `renameat_with(..., RenameFlags::EXCHANGE)`，不引入新的 domain 或 general I/O abstraction。HTTP、forward migration/backup、FTS schema 与其余 durable-domain dependency decisions 继续由后续交付决定。
