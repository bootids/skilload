...
对于原本不存在的 `data/skilload.db`，持久 import 在完成 input/schema/domain/conflict planning 后，在同一 data directory 建立 restrictive staging database；只有 schema 和 SQLite `COMMIT` 成功、SQLite connection 已关闭且根重新验证后才尝试发布 live database。发布不得提前以 live `skilload.db` 名称创建空 guard：它从 held data-directory descriptor 以 `linkat` 直接将已验证、已 sync 的 staging inode 链接到尚不存在的 `skilload.db`。该 link 成功时 live name 已经是完整 committed generation；竞争方已创建 target 时 link 失败为 typed identity drift，绝不覆盖其文件。每个 post-link hook 与最终身份检查都必须比较 live entry 和 held staging inode；若被替换，未知 target 保留并返回 error，不报告 success。每个 SQLite persistence error exit 以及 pre-COMMIT rollback 前，staging 以 no-follow descriptor 记录当时仍与其 held FD 绑定的 `-journal`、`-wal`、`-shm` sidecar；Drop 只删除这些记录的 sidecar、held staging、已发布但尚未完成的 held live link、database lock 与空 data/state directory。pre-commit hook 或未知来源在记录后留下的同名 sidecar 不被推断为 owned，保留并阻止删除所在目录。若 `COMMIT` 已完成但文件或父目录 sync 失败，命令返回 error 且不声称 Library 仍不存在。
...
所有 publish hook 或最终验证完成后，first import 必须再次比较 held staging FD、data-directory descriptor 与 live `skilload.db` entry，才可删除原 staging link 并将它标为 published。link 后任一不匹配都返回 typed drift error，且未完成的 staging Drop 只移除仍为 held inode 的 live link；外部 replacement 必须保留。发布后完成 parent/data-directory 与新建目录 sync 后，first import 必须最后一次比较 live `skilload.db` entry 与 held staging FD，任何 drift 都返回 typed error 而不报告 `changed`。未完成的 staging Drop 只按 held identity 清理原 staging、live link 和已记录 sidecar，再让创建目录的 RAII guard 删除仍为空且当前 identity 匹配的根；未证明属于本调用的同名 sidecar 保留。

为避免 source staging pathname 在 final check 后被替换，P2 只以 descriptor-relative `linkat` 从已验证 staging entry 直接建立 final `skilload.db` link；它不再创建随机 publication placeholder、第二个 publication link 或 zero-byte live guard。`linkat` 的 target-exists failure 保留 foreign database；成功后的 identity revalidation 与 Drop cleanup 仅操作仍匹配 held staging inode 的两个 link。正常成功会按 held identity 删除原 staging link。

Library export 在创建 staging 文件前比较 no-follow output target 与有效 Library database generation（database、WAL、SHM）及 database lock，拒绝任何碰撞。其他 target 使用同目录 staging、file sync 与 parent-directory sync；父目录以 no-follow descriptor 打开并绑定 device/inode。export 对既有 regular output 记录 identity；对 absent output 用 `O_EXCL|O_NOFOLLOW` 创建 held empty guard。创建并验证随机 publication link 后，所有 hook 完成时必须同时验证 held staging、publication link 和 output guard，再以 descriptor-relative `RenameFlags::EXCHANGE` 发布。交换后若 output 不是 held staging 或 publication 不是 guard，立即反向交换恢复旧 output/absence 并返回 error；成功时只删除仍匹配的旧 output 或本调用 guard。parent sync 后仍须比较 held staging FD 与 output entry。若检测到父目录、publication link 或 output identity drift，命令返回错误而不报告成功，且不删除未知 replacement。rename 前失败保留旧 target 或无 target并清理已证明所有权的 staging，rename 后 parent sync failure 同样返回错误但本调用文档可能已经被外部移动。成功仅在最终 sync 和 output identity 验证完成后返回。

任何 exchange 前的导出发布失败都必须按持有暂存 inode 分别清理原暂存名称和随机 publication 名称；absent target guard 也只在 entry 仍匹配其创建 identity 时删除。exchange 后 identity mismatch 必须先尝试反向交换；无法证明所有权的 publication、output 或 guard replacement 一律保留。
...
P2 默认测试使用 temporary XDG/HOME roots、bundled SQLite 与 generated input；它们覆盖 Unicode 15.1.0 tag normalization、canonical source/positive portable evidence、六种 non-model import ceiling 与 API-v2 `library_input_limit_exceeded`、combined 10,000-entry/byte transfer closure、duplicate keys、nonregular/identity-drift input、first-import staging cleanup、foreign-sidecar preservation 与 orphaned database sidecar rejection、pre-open 与 read-only SQLite ABA inode replacement、直接 live database link 和已发布 database replacement、SQLite transaction rollback、post-commit error、既有数据库 no-follow/identity race、缺失 schema 列、foreign-key parent-key mismatch 与超出 API-v2 `UInt` 的损坏诊断、state revision 溢出拒绝、deterministic export、database/WAL/SHM/lock collision、held parent descriptor 的 rename race、publication-link/output replacement、parent sync failure 与 CLI observed/changed/unchanged projection。

发布 regression 必须证明 first import 在 live name 出现时已经是 committed SQLite generation，并在 link 后、final identity check 后的 database replacement 中保留 foreign target；sidecar tests 同时证明每个 SQLite persistence failure path 的已记录 sidecar 被清理，而 pre-commit foreign sidecar 保留。

## Decisions Deferred Beyond the Configuration Foundation

`PLAN-0002` 固定 Rust toolchain、`clap`、`serde`、TOML、error、filesystem-staging、JSON 与 test dependencies。`PLAN-0003` 将 `rusqlite 0.40.2`（`default-features = false`、`bundled`）、`unicode-normalization =0.1.23`、`libc =0.2.189` 与 `rustix =1.1.4`（`fs`）锁入 workspace，并固定 P2 SQL names 与 local Unicode 15.1.0 数据；`rustix` 提供 first-import 直接发布所需的 descriptor-relative `linkat`，以及 export 既有 guard/exchange 所需的 `O_EXCL|O_NOFOLLOW` 与 `renameat_with(..., RenameFlags::EXCHANGE)`，不引入新的 domain 或 general I/O abstraction。HTTP、forward migration/backup、FTS schema 与其余 durable-domain dependency decisions 继续由后续交付决定。
