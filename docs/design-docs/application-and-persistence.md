...
`PLAN-0005` 的规划基线把下一次可执行升级固定为 SQLite schema v2：新增普通 content-bearing `library_fts` FTS5 virtual table，不改变 v1 的 base Library tables。该计划仍处于 `plan`，所以本段只记录已决定的实现方法，不表示当前二进制已经创建索引、迁移数据库或暴露读取命令。
...
Schema v2 二进制必须继续以 read-only 方式读取完整、可验证的 v1 base rows，使 `library list`、`library get` 和 `library export` 可在升级前提供本地观察或可移植逃生路径；依赖 FTS 的 `library search` 与所有 database writes 返回 `migration_required`，直到显式 `doctor --fix` 在持久 database lock 下完成 backup 和 v1→v2 transaction。默认 `doctor` 只读：它以 read-only source connection 向内存 SQLite destination 运行 online backup，并在该副本上执行需要 writable connection 的 FTS5 `integrity-check`，不得为诊断创建 XDG roots、backup 或 journal。`doctor --fix` 只对已证明 base rows 完整的 v1 migration 或 FTS-only drift 执行 action；前者不改变 product `state_revision`，后者也只重建派生数据。
...
`PLAN-0002` 固定 Rust toolchain、`clap`、`serde`、TOML、error、filesystem-staging、JSON 与 test dependencies。`PLAN-0003` 将 `rusqlite 0.40.2`（`default-features = false`、`bundled`）、`unicode-normalization =0.1.23`、`libc =0.2.189` 与 `rustix =1.1.4`（`fs`）锁入 workspace，并固定 P2 SQL names 与 local Unicode 15.1.0 数据。`PLAN-0005` 的规划基线选择为同一 `rusqlite 0.40.2` 启用 `backup` feature，并加入 `sha2 0.11.0` 以流式计算 migration manifest 的 SHA-256；它不增加 HTTP、异步 runtime、通用 migration framework 或第二个 database owner。FTS schema v2、v1→v2 backup/migration 和当前 durable database 的 doctor repair 将在该 Plan 获得明确执行授权后实现；HTTP 与其余 durable-domain dependency decisions 继续由后续交付决定。
