...
Before every manifest create, replacement, or deletion, invoke the resolved Git from the worktree root with `GIT_LITERAL_PATHSPECS=1` and fixed `git ls-files --error-unmatch -- <worktree-relative-manifest>` arguments. Exit success means tracked and produces `tracked_local_manifest`; the expected no-match result means untracked. Perform the check in preflight, revalidate immediately before rename/unlink, and include the observed Git-index identity in the final transaction baseline. If tracked at either check, do not touch the manifest, links, database/index, pending journal, or owned exclude entry; report an exact `git rm --cached -- <path>`-style remedy using safe rendering. Journal recovery uses the same gate and remains pending until the user untracks the path. The same-account concurrency exclusion applies after final revalidation; skilload does not claim to lock Git's index against a concurrent user process.
