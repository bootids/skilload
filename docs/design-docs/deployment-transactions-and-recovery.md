...
Use a bounded YAML event parser followed by schema deserialization and canonical serialization. Read at most 1,048,577 bytes so the reader can reject rather than allocate a file beyond the 1,048,576-byte ceiling. Require one UTF-8 document, at most 200 records, 4,096 scalar/mapping/sequence nodes, eight open mapping/sequence containers, and 4,096 UTF-8 bytes per scalar. Configure the event phase never to resolve aliases and reject anchors, aliases, explicit tags, directives, additional documents, duplicate mapping keys, and non-string mapping keys. Increment/check each counter before retaining the event; only a fully accepted stream may construct the config or lock model. A conceptual version-1 config is:
...
The implementation may add explicitly specified format fields only within those Revision 1 ceilings, and never machine paths, timestamps, Trust, Agent selections, or Library IDs. Portable YAML and JSON encode a `RepositoryId(u64)` as a quoted decimal string so consumers cannot lose precision. Canonical serialization uses LF endings, two-space indentation, stable key order, a final newline, and source-sorted records. Comments and input formatting are not preserved. Validate both documents before source resolution, Trust lookup, network access, journal recovery that depends on their contents, or full document allocation.
...
Determine repository membership through the clean Git runner from `github-resolution-and-integrity.md`, so inherited `GIT_*` values cannot select a different repository or helper. From the workspace, use fixed noninteractive `rev-parse` arguments to resolve the canonical worktree root, absolute per-worktree Git directory, effective index path, and effective `info/exclude` path. Require the canonical workspace to be the root or a descendant of the returned worktree root; interpret relative Git-path results against Git's invocation directory, canonicalize them, and record file/directory identities for revalidation. Do not construct `<workspace>/.git/info/exclude` or `<worktree>/.git/index`, because a linked worktree has a `.git` file, a per-worktree index, and normally resolves the exclude path into its common repository.
...
Before every manifest create, replacement, or deletion, revalidate the recorded worktree/Git-directory/index identities and invoke the resolved Git with the fixed validated exec path, explicit `--git-dir=<per-worktree-git-dir>` and `--work-tree=<canonical-worktree>` arguments, application-owned `GIT_INDEX_FILE=<effective-index>` and `GIT_LITERAL_PATHSPECS=1`, and fixed `ls-files --error-unmatch -- <worktree-relative-manifest>` arguments. No caller `GIT_*` value survives. Exit success means tracked and produces `tracked_local_manifest`; the expected no-match result means untracked. Perform the bound check in preflight, repeat it immediately before rename/unlink, and include all three repository-resource identities in the final transaction baseline. If tracked or any identity drifted at either check, do not touch the manifest, links, database/index, pending journal, or owned exclude entry; report an exact `git rm --cached -- <path>`-style remedy or stale-resource retry using safe rendering. Journal recovery uses the same gate and remains pending until the user untracks the path. The same-account concurrency exclusion applies after final revalidation; skilload does not claim to lock Git's index against a concurrent user process.
...
* `tracked_local_manifest`: Git currently tracks machine-local deployment state; hard failure for every manifest mutation/recovery until the exact path is removed from the index.
...
`workspace sync` reads a complete valid lock, requires explicit Agents, and plans the entire locked set for each selected Agent. It removes obsolete exact owned links for those Agent profiles and creates/restores required cache links. Agent selection is not written to portable config. When the desired set is empty and the plan only removes exact owned links/target records, validated Agent roots plus filesystem/durable ownership are sufficient; the executable/settings/conflict preflight is omitted and revoked Trust does not block that explicit cleanup. Any required create, replacement, repair, restoration, content use, or mixed action requires active Trust and normal functional preflight for every affected source.
...
Before any mutation promotes a new object, quota planning measures filesystem-allocated bytes for promoted objects, retained quarantine payloads/manifests, and retained staging files in checked `u64` arithmetic; manifest allocation and filesystem rounding count, while the rebuildable cache index does not. It calculates protected bytes, reclaimable LRU bytes, requested bytes, removals, and one complete stable post-recovery projection for the whole batch. Overflow is a typed validation failure, never saturation or wraparound. The absent configuration default is 536,870,912 bytes, and persisted TOML limits stop at positive signed-64-bit maximum. A valid positive-`u64` `--cache-limit-bytes` value replaces the configured limit only for that invocation and must be at least the configured value; planning rejects the flag on non-promoting operations. Ordinary promotion staging stays within the effective limit. The plan prunes before committing domain state and binds configured/effective/projected/stable-quarantine/repair-headroom/override values into confirmation and the authoritative API-v1 `CacheQuota` result, whose byte fields are decimal strings. Nothing writes the override back to configuration or later domain state.

## Corruption and Cache Miss

Every promotion, link creation/replacement, and mutating use verifies the cache manifest and expected integrity before exposing or reusing a target. On mismatch, hold the cache mutation lock, write one repair journal, and atomically rename exactly that object under `cache/quarantine/<uuid>` without copying it. Only this serialized repair may exceed the effective quota. Its physical headroom is capped at the corrupt object's allocated bytes plus 16,777,216 bytes for replacement pack/index/manifest overhead, and preflight requires filesystem-reported free space for the replacement plus overhead before receive begins. Insufficient space returns `cache_repair_space_insufficient` without changing a lock or pin.

Fetch the same repository ID/commit/path at most once and promote only the expected digest. Success deletes the complete quarantined payload and every repair staging file before dependent links or product state commit. Mismatch, unavailable commit, or capacity failure deletes all replacement staging and the full corrupt payload, retains at most a 65,536-byte diagnostic quarantine manifest, and preserves the pin. Recovery finishes these same cleanup rules before admitting another cache mutation; it never starts a second repair or accumulates full quarantines. Every completed or recovered boundary therefore returns to the stable quota projection, even though cache info exposes temporary/stable quarantine and staging while repair is pending. Read-only status, info, and doctor observers report a mismatch without moving or refetching the object. Never change a workspace/global pin because remote bytes differ. A deployed native symlink is read directly by the Agent, so skilload cannot prevent a same-account edit or disk fault from being consumed between the modification and the next integrity observation; the guarantee begins again once skilload detects the mismatch and refuses to reuse it as valid.
...
Status joins desired/pinned, ownership, and observed state into typed findings: healthy, missing, stale, degraded name conflict, foreign exact, drifted owned, disabled, inaccessible, detached orphan, relocation required, tracked local manifest, duplicate workspace instance, cache missing/corrupt, Trust blocked, and recovery pending. Detached orphan rows are warnings, not active desired targets. Default workspace status reports every registered deployment for that exact workspace; when the current manifest's instance matches one old record but canonical paths differ, it reports the API-v1 `WorkspaceRelocation` containing old/current `PathValue` fields, the matched instance ID, required Agents, and `workspace sync --rebind-from` guidance without changing either record. It emits null relocation evidence for every other result rather than returning an incomplete candidate. An explicit option may rerun current Agent preflight.

Doctor uses the same observers across global database, cache, manager, profiles, journals, and known workspaces. `--fix` invokes only repair actions with complete local proof: finish/rollback a journal, rebuild FTS, apply the backed-up forward database migration from `SKL-OPS-003`, recreate an exact link from a verified cache object when exact Trust is active, or reconstruct a workspace manifest when all fields match. It never fetches or adopts, and revoked Trust turns a missing external link into a reported blocker rather than a repair action. Base-database corruption blocks writes and returns `DatabaseCorruptDetails` naming `database-corruption-v1`; backup selection, isolated validation/migration, atomic replacement, explicit destructive reset, ownership re-establishment, and rollback follow [`../product-specs/database-recovery.md`](../product-specs/database-recovery.md), never an implicit database recreation.

## Failure-Injection Acceptance

The transaction adapter exposes failpoints after every journal write, rename, database commit, and verification. Tests execute each operation with every failpoint, allow direct pre-recovery inspection to observe a journal-described transitional state, restart a fresh application, run recovery, and then assert one coherent old/new state, no unowned change, no dangling temporary target, and a complete/blocked journal explanation. Workspace parser fixtures exercise every exact byte/record/node/depth/scalar boundary plus alias, anchor, tag, directive, multi-document, duplicate-key, non-string-key, and malformed-UTF-8 rejection before model construction. Git exclusion fixtures cover root and nested workspaces, literal metacharacters, ordinary repositories, and linked worktrees; `git check-ignore -v` must identify the exact manifest through the Git-resolved exclude file while a root-level lookalike and neighboring paths remain visible and user lines remain unchanged. A tracked-manifest matrix force-adds plain and metacharacter paths before ordinary sync and every recovery phase, verifies no manifest/link/database/journal/exclude drift, untracks the path, and proves retry convergence. Relocation matrices cover same-filesystem rename, copy-then-remove moves with preserved symlinks, changed relative-link resolution, different/same/missing exclude files, both Agents, missing Agent selection, accessible/inaccessible old paths, duplicate manifests, changed lock/transaction/link evidence, foreign new targets, and every rebind failpoint; only the fully proved case ends with one new registration and no old owned path/pattern. Near-quota cache fixtures include retained staging/quarantine allocation, one 500-MiB corrupt object under the default limit, exact-success cleanup, insufficient free space, replacement mismatch, repeat failure, and interruption at every repair phase; a completed/recovered fixture has at most one replacement or one 65,536-byte diagnostic manifest and no second full copy. Database fixtures validate manifest digests, schema versions, foreign keys, stale WAL exclusion, live-file drift, atomic restore rollback, and explicit-reset non-adoption under the normative recovery procedure. Multi-Agent and multi-source batches receive the same matrix.
