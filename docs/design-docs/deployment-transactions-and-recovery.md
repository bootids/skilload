...
The manifest contains workspace canonical path, lock digest, and for each workspace target keyed by `(canonical workspace, Agent, canonical project Skill root)`: replaceable environment fingerprint/observations, exact link path, expected cache target, source, repository ID, commit, integrity, and last committed transaction ID. An environment-only change updates observations on the same target; it never creates competing ownership for an unchanged root. The durable `known_workspaces`/`owned_links` rows mirror enough data to find and clean known deployments when the workspace manifest is absent or inaccessible.
