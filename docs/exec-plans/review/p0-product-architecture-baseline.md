...
The selected Agent executable must be installed and resolvable. There is no numeric Agent-version gate, but functional preflight checks directories, Agent disable settings, ownership, and conflicts. Workspace deployment observations are tied to current `HOME`, `CLAUDE_CONFIG_DIR`, and `CODEX_HOME`; state records resolved roots and promises apply only to that environment. Global and manager profile identity is narrower: only Agent plus canonical global Skill root identifies the target, while the other environment values remain replaceable observations. Changes apply on the next Agent launch, not through hot reload. A Git workspace resolves its effective `info/exclude` file through Git, maintains only exact owned entries there, and never edits shared `.gitignore`; this includes linked worktrees whose `.git` is a file. A git-excluded local manifest plus a global workspace index record exact links, cache targets, Agents, lock digest, and environment. A missing manifest can be reconstructed only when link name, target, commit, and integrity exactly match expected state. Status is read-only and reports registered deployments. Deleting a workspace record succeeds only when config is empty and no managed deployment remains; selected prior Agent deployments must first be synced to remove their links. The supported workspace size target is 200 Skills.
...
External cache entries are content-addressed by repository identity, commit, and Skill path and are never edited in place by skilload. Active verified workspace/global links protect their targets from prune; a lockfile or detached-orphan record alone does not pin cache content. The configured quota defaults to 536,870,912 bytes. Cache-promoting operations may explicitly supply `--cache-limit-bytes <BYTES>` as a finite whole-invocation effective ceiling; it is confirmation-bound, visible in results, and not persisted. Before a state mutation exceeds the effective quota, skilload prunes unprotected least-recently-used entries and otherwise fails without product-state commit. Cache clear preflights and removes verified managed workspace/global links before deleting external content. Inaccessible or mismatched known workspaces block by default; force may leave an orphan or broken link. Library, Trust, workspace config/lock, global desired state, and built-in manager copies survive. On detected corruption, a mutating use quarantines the entry, refetches the same commit once, and rechecks integrity; persistent mismatch fails without rewriting a lock. Because native Agents read deployed links without passing through skilload, a same-account post-deployment modification may be consumed before the next integrity observation; skilload never promotes, links, or reuses the object after detecting that mismatch.
