...
- [x] (2026-08-18 05:12Z) Synchronized all four fixes across product behavior, architecture, persistence/deployment/Agent/CLI designs, tests, and this Plan; focused contradiction checks and the complete documentation acceptance gates pass; next is the preliminary ledger commit/push before GitHub replies.
...
`Source`: inline thread `PRRT_kwDOT7YN2s6Z_IxA`, comment `PRRC_kwDOT7YN2s7ikH7c`, https://github.com/bootids/skilload/pull/1#discussion_r3801120476. `Problem`: `SKL-CACHE-003` promises a configured quota and explicit over-capacity override without defining the default value or the override's CLI spelling, scope, and lifetime, leaving incompatible Revision 1 behavior possible. `Disposition`: fixed. `Status`: open. `Resolution`: set 536,870,912 bytes as the absent/unset default; defined `config set|unset cache_limit_bytes`, the exact finite `--cache-limit-bytes` flag and permitted cache-promoting operations, whole-invocation scope, confirmation binding, JSON fields, and non-persistence; synchronized `SKL-CACHE-003`, `SKL-OPS-006`, `SKL-TRUST-004`, persistence, deployment, CLI/test design, and Plan context. `Evidence`: preliminary remediation commit to be recorded after push; focused quota searches agree across all six changed behavior/design surfaces, and the complete behavior-structure, link, ASCII, command-tree, removed-command, and whitespace gates pass. `GitHub outcome`: no reply yet; thread resolved: false.

### RC-006 - Key a global profile by Agent and deployment root

`Source`: inline thread `PRRT_kwDOT7YN2s6Z_IxD`, comment `PRRC_kwDOT7YN2s7ikH7g`, https://github.com/bootids/skilload/pull/1#discussion_r3801120480. `Problem`: the Agent design maps a broad environment fingerprint to `profile_id`, contradicting `SKL-GLB-002` and allowing multiple profile identities to own the same Agent/global-root target when only an auxiliary compatibility root changes. `Disposition`: fixed. `Status`: open. `Resolution`: made `(Agent kind, canonical resolved global Skill root)` the only profile identity/uniqueness tuple across `SKL-GLB-002`, persistence, Agent design, and Plan context; retained executable, HOME, Agent configuration, compatibility roots, and fingerprint as replaceable observations; added the fixed-HOME/changed-`CODEX_HOME` acceptance and test fixture. `Evidence`: preliminary remediation commit to be recorded after push; focused identity searches find no fingerprint-to-profile mapping and all complete documentation gates pass. `GitHub outcome`: no reply yet; thread resolved: false.

### RC-007 - Detach inaccessible global targets safely

`Source`: inline thread `PRRT_kwDOT7YN2s6Z_IxG`, comment `PRRC_kwDOT7YN2s7ikH7l`, https://github.com/bootids/skilload/pull/1#discussion_r3801120485. `Problem`: `SKL-GLB-009` blocks uninstall when a stored target is inaccessible, but that association then also blocks every shared-pin update and no listed command can abandon the permanently unavailable target. `Disposition`: fixed. `Status`: open. `Resolution`: defined `global uninstall --profile <id> --detach-inaccessible` only for one stored profile proven inaccessible; it moves selected associations from active desire into durable detached-orphan evidence, performs no target write/deletion, reports `link_removed: false`, excludes the orphan from update/pin and prune protection, keeps it visible to reads, and permits later exact cleanup without weakening foreign-path ownership. `Evidence`: preliminary remediation commit to be recorded after push; focused flag/orphan searches agree across product, persistence, Agent, deployment, CLI, and Plan surfaces, and all complete documentation gates pass. `GitHub outcome`: no reply yet; thread resolved: false.

### RC-008 - Qualify integrity after native-link deployment

`Source`: inline thread `PRRT_kwDOT7YN2s6Z_IxH`, comment `PRRC_kwDOT7YN2s7ikH7m`, https://github.com/bootids/skilload/pull/1#discussion_r3801120486. `Problem`: `SKL-CACHE-005` and the architecture promise that modified cache content is never served, but an Agent reads the direct cache symlink without a skilload launch-time validation path and can consume a post-deployment modification before the next skilload check. `Disposition`: fixed. `Status`: open. `Resolution`: changed physical immutability to skilload-managed immutability with non-writable permissions as defense in depth; retained verification before promotion/linking/mutating reuse, read-only mismatch reporting, and quarantine/one exact refetch; explicitly documented in `SKL-CACHE-001`, `SKL-CACHE-005`, architecture, Agent/deployment designs, tests, and Plan that skilload cannot intercept native reads between a post-deployment local change and its next integrity observation. `Evidence`: preliminary remediation commit to be recorded after push; focused integrity-language searches find the limitation and no remaining absolute "never served" contract outside the historical problem statement, and all complete documentation gates pass. `GitHub outcome`: no reply yet; thread resolved: false.
